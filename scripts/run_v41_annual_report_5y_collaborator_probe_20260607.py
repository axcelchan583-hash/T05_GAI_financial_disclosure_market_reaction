#!/usr/bin/env python3
"""Five-year annual-report broad-collaborator probe for v36 GenAI events.

This run builds a pre-event broad collaborator set from all focal-firm annual
reports disclosed in the five years before each GenAI event. It then compares
the stock-market reaction of these listed collaborators with the existing
product-market competitor panel.
"""
...
RUN_ID = "v41_annual_report_5y_collaborator_probe_20260607"
OUT_DIR = ROOT / "results" / RUN_ID
DOC_PATH = ROOT / "docs" / "empirical_runs" / "105_v41_annual_report_5y_collaborator_probe_20260607.md"
...
def select_pre_event_reports(events: pd.DataFrame, annual_index: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, event in events.iterrows():
        ...
        if candidates is None or candidates.empty:
            continue
        window_start = event_date - pd.DateOffset(years=5)
        strict = candidates[
            (candidates["disclosure_date"] < event_date)
            & (candidates["disclosure_date"] >= window_start)
        ].copy()
        ...
        if strict.empty:
            continue
        strict = strict.sort_values(["disclosure_date", "report_year"], ascending=[False, False]).reset_index(drop=True)
        for report_rank, (_, report) in enumerate(strict.iterrows(), start=1):
            chosen = report.to_dict()
            row = {
                **event.to_dict(),
                **{f"report_{k}": v for k, v in chosen.items()},
                "report_rank_by_recency": report_rank,
            }
            rows.append(row)
...
def annual_source_summary(
    ...
    rows = [
        ...
        {
            "layer": "annual_report_index_2017_2025",
        ...
        {
            "layer": "selected_5y_pre_event_reports",
...
def write_report(
    ...
    text = f"""# v41 Five-Year Annual-Report Collaborator Probe
...
- `results/{RUN_ID}/selected_5y_pre_event_annual_reports.csv`
...
def main() -> None:
    ...
    print(f"selected_5y_pre_event_reports={len(selected_reports):,}, events={selected_reports['event_key'].nunique() if not selected_reports.empty else 0:,}")
    ...
    selected_reports.to_csv(OUT_DIR / "selected_5y_pre_event_annual_reports.csv", index=False, encoding="utf-8-sig")
